...
export default async function (fastify, options) {
  ...
  fastify.post('/:id(\\d+)', { preHandler: [fastify.auth] }, async (req, reply) => {
    ...
    if (friend.error === true) {
      switch (friend.msg) {
        case 'unknowUser':
          return reply.code(404).send({ error: true, code: 'USER_INVALID_ID', info: 'unknow user id' });
        case 'alreadyFriend':
          return reply.code(404).send({ error: true, code: 'USER_ALREADY_FRIEND', info: 'user already friend' });
        case 'friendLimit':
          return reply.code(404).send({ error: true, code: 'MAX_FRIEND_LIMIT', info: 'limit friend hit' });
      }
    }
