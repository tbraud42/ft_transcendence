...
import {
    ...
    RenderOptions, LeafAddress, ClientStatus
} from './types';
...
export function mapRound1(snap: Snapshot, max: number) {
    const raw = (snap.matches || []).filter(m => (m.round ?? 1) === 1);
    const r1: Match[] = [];
    for (let k = 0; k < max; k++) {
        r1[k] = raw[k] || { id: `ph-${k}`, round: 1, p1: null, p2: null, status: 'waiting' };
    }
    return r1;
}
...
export function decorateLeaves(
    ...
    const decorate = (
        ...
        if (label) {
            ...
            if (p && p.connected) {
                v = ClientStatus.CONNECTED;
            }
            if (opts.myUsername && label === opts.myUsername) {
                const ready = !!opts.isReady?.({ side, pair: pairIdx, pos: posName });
                if (ready) {
                    v = ClientStatus.READY;
                }
            }
            if (p && !p.connected) {
                v = ClientStatus.OFFLINE;
            }
        }
        ...
        const ownerByFlag = !!opts.isOwner;
        const ownerByCreator = !!snap.creator?.username && opts.myUsername === snap.creator?.username;
        const ownerByOwner = !!snap.owner && opts.myUsername === snap.owner;
        const isOwner = ownerByFlag || ownerByCreator || ownerByOwner;

        if (!label && isOwner) {
            const b = button('Add bot',
        ...
        if (label && opts.myUsername === label) {
            ...
            const b = button(ready ? 'Ready ✔' : 'Ready', cls);
            b.onclick = e => {
    ...
    for (let i = 0; i < perSide / 2; i++) {
        const leftPair  = r1[i * 2]     || { id: `ph-L-${i}`, round: 1, p1: null, p2: null, status: 'waiting' };
        const rightPair = r1[i * 2 + 1] || { id: `ph-R-${i}`, round: 1, p1: null, p2: null, status: 'waiting' };
